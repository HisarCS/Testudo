...
def main():
    ...
    try:
        while True:
            keyword_detected = keyword_detected_queue.get()  # blocks until True
            if keyword_detected:
                ...
                # 2) Run the conversation flow with the mic
                conversation_flow()

                # 3) Set an ignore period so we don't immediately re-trigger
                set_ignore_until(time.time() + 2.5)

                # 4) Re-initialize background listener so we can detect 'testudo' again
                #    This creates a new mic stream in the background.
                stop_listening_fn = init_background_listener(
                    keyword="testudo",
                    event_queue=keyword_detected_queue,
                    mic_index=2
                )
                print("[Main] Listening again for 'testudo'...")
