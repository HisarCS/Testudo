...
from movement_utils.simple_servo import move_back, move_forward
...
def call_lm_studio(
    ...
    system_prompt = (
        "TESTUDO is an affordable, open-source, AI-integrated quadruped robotic companion. "
        ...
        "You are the AI of Testudo while the user talking to you is a human."
        ...
        "By the way I know you only have knowledge up to 2021 but you are a model living in the year 2025. Be aware of this when responding to the user"
        "Don't mention Testudo unless the user asks about you or about Testudo."
    )
...
def conversation_flow():
    ...
    Additionally, if the user’s speech contains movement trigger words,
    the appropriate movement function is executed on a separate thread.
    """
    ...
    # Check if the user command contains any movement trigger words
    lower_input = user_input.lower()
    if any(word in lower_input for word in ["move", "go", "come"]):
        # If the command also includes a "back" keyword, move backward; otherwise, move forward.
        if any(word in lower_input for word in ["back", "backward", "reverse"]):
            move_func = move_back
        else:
            move_func = move_forward
        movement_thread = threading.Thread(target=move_func)
        movement_thread.daemon = True  # Allow thread to run independently
        movement_thread.start()

    lmstudio(user_input)

    # Continue until the user doesn't speak for 7 seconds.
    while True:
        user_input = continuous_recognition(
            mic_index=get_input_device_index(),
            timeout=7,
...
def lmstudio(user_input):
    ...
    print("USER DATA STR: ", user_data_str)
