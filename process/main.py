...
import requests
import queue
from process.helpers.speech_helper import (
    init_background_listener,
    continuous_recognition,
    set_ignore_until
)
from process.helpers.text_to_speech_helper import text_to_speech
...
def start_rasa_servers():
    ...
    action_process = subprocess.Popen(
        [
            "rasa", "run",
            "actions",
            "--actions", "models.model_v1.actions.actions",
            "--debug"
        ],
        text=True
    )
...
def wait_for_action_server_any(url="http://localhost:5055/", max_retries=60):
    ...
    for attempt in range(max_retries):
        try:
            ...
            time.sleep(5)  # Give it extra time after the first response
            return
        except requests.exceptions.RequestException:
            pass
        time.sleep(1)
